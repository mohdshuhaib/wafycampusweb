...
import { Droplets, Plus, Trash2, AlertCircle, FileUp, MapPin, Edit2, X, Search } from 'lucide-react';
...
export default function ManagePlacesClient({
  initialPlaces
}: {
  initialPlaces: Place[];
}) {
  ...
  const confirmDelete = async () => {
    ...
    startLoading();
    const { error } = await supabase.from('cleaning_places').delete().eq('id', placeToDelete);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-foreground tracking-tight">Manage Places</h1>
          <p className="text-sm text-muted-foreground">Add or remove campus cleaning places and configure capacities</p>
        </div>
      </div>
      ...
      <div className="bg-card text-card-foreground border border-border rounded-lg p-5 sm:p-6 shadow-xs">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
          <h2 className="text-lg font-semibold flex items-center gap-2 text-foreground">
            <Droplets className="w-4 h-4 text-primary" /> Active Places ({initialPlaces.length})
          </h2>
                    ...
                    <div key={p.id} className="flex justify-between items-center p-3.5 bg-muted/30 rounded-md border border-border hover:border-border/80 transition-colors">
                      <div>
                        <h4 className="font-medium text-sm text-foreground">
                          {p.name} <span className="text-[11px] font-normal px-2 py-0.5 bg-secondary text-secondary-foreground rounded-sm ml-1.5">{p.floor || 'Ground'}</span>
                        </h4>
                      ...
                      <div className="flex gap-1.5">
                        ...
                        <button onClick={() => setPlaceToDelete(p.id)} className="p-1.5 text-destructive hover:bg-destructive/10 rounded-md transition-colors" title="Delete Place">
      ...
      {placeToDelete && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-background/80 backdrop-blur-xs p-4 animate-in fade-in" onClick={() => setPlaceToDelete(null)}>
          <div 
